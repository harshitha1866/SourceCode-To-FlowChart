...
public class Digraph {
    ...
    public Node addNode(String nodeID, String nodeName, String shape) {
        if (exists(nodeID)) {
            System.out.println("Error: Node " + nodeID + " already exists.");
            System.exit(0);
        }
        Node n = new Node(nodeID, nodeName, shape);
        n.graph = this;
        nodes.add(n);
        return n;
    }
    ...
    public void generate(String filename) {
        try {
            ...
            for (Node n : nodes) {
                if(n.hasShape()){
                    if (n.hasName())
                    writer.println("ID" + n.nodeID + " [shape=" + n.shape + " label=\"" + n.nodeName + "\"];");
                else
                    writer.println("ID" + n.nodeID + " [shape=" + n.shape + " label=\"" + n.nodeID + "\"];");
                }
                else{
                if (n.hasName())
    ...
    class Node {
        ...
        public Node(String nodeID, String nodeName, String shape) {
            this.nodeID = nodeID;
            this.nodeName = nodeName;
            this.shape = shape;
        }
